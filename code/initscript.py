...
def main(argv):
   ...
   try:
      opts, args = getopt.getopt(argv,"h",["add_uvws","miriad_to_uvfits","sudo_miriad_to_uvfits","add_HERA","importuvfits","swap_ant","del_uvfits","del_uvcU","del_ms","split_sim_ms"])
   except getopt.GetoptError:
      print 'python initscript.py -h --add_uvws --miriad_to_uvfits --sudo_miriad_to_uvfits --add_HERA --importuvfits --swap_ant --del_uvfits --del_uvcU --del_ms --split_sim_ms'
      sys.exit(2)
   for opt, arg in opts:
      #print "opt = ",opt
      if opt == '-h':
