...
\textbf{TLG: Due to the course angular resolution of HERA-19, the Galactic centre is almost unresolved and can therefore be used to perform a first order calibration of the array - first order still seems out of place (would rather use initial and mention that it can be refined).}
We leverage on the fact that at the limit angular resolution of HERA-19, the Galactic centre is almost unresolved and can be used to calibrate the array to a first order. We selected the snapshot (i.e. 10 minute long) observation when the Galactic centre transits and model it as an unresolved 1~Jy source with a flat spectrum in the HERA-19 band. 
\textbf{TLG: We calibrated by first determining the antenna--based delays using the CASA (ref) task {\it gaincal}, thereafter we computed the complex gains as a function of frequency usisng the CASA task {\it bandpass}.}
We calibrated by solving for antenna--based delays first and the complex gains as a function of frequency usisng the CASA (ref) task {\it gaincal} and {\it bandpass} respectively. Visibilities were then Fourier transformed and deconvolved using the CASA task {\it clean} (Figure~\ref{fig:gc_first_cal}).
