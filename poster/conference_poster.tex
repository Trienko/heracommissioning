...
%      \includegraphics[width=0.4\linewidth]{Layout.png}%
      \includegraphics[width=0.4\linewidth]{delay.png}%
      \label{fig:evaluation:avgPrice}%
    }
    \caption{Right: an aerial view of HERA-19 at the Karoo site in South Africa. Left: example of delay solutions for a subset of HERA antennas. Data were taken on {\bf (GB: include the calendar day)} (blue) and ??? (red) and ??? (green) days afterwards respectively. {\bf (GB: can you calculate the rms difference of the delays? It could be a useful number to quote).}}
  \label{fig:HERA19}  
  \end{minipage}
...
\captionof{figure}{Absolutely calibrated, Healpix re-sampled image at 150~MHz combined three nights of data. The units of the colorbar is Jy~beam${-1}$.}
\label{fig:all-sky}
\end{center}\vspace{1cm}
